...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        spent_calories =  (coeff_callories_walk_1 * self.weight +
                           (self.get_mean_speed() ** 2 // self.height) *
                           coeff_callories_walk_2 *self.weight) * self.duration
        return spent_calories
...
class Swimming(Training):
    ...
    def get_mean_speed(self) -> float:
        """Получить среднюю скорость движения."""
        sr_speed = self.length_pool * self.count_pool /\
...
def read_package(workout_type: str, data: Dict) -> Training:
    ...
    types_of_workout: Dict[str, type] = {'SWM' : Swimming,
                                                               'WLK': SportsWalking,
                                                              'RUN' : Running}
